...
if 0
kount=(u(edge_u_ij)~=0)+(u(edge_u_ijm1)~=0);
up(poly_ij)=(u(edge_u_ij).*(u(edge_u_ij)~=0)+u(edge_u_ijm1).*(u(edge_u_ijm1)~=0))./kount;
kount=(v(edge_v_ij)~=0)+(v(edge_v_im1j)~=0);
vp(poly_ij)=(v(edge_v_ij).*(v(edge_v_ij)~=0)+v(edge_v_im1j).*(v(edge_v_im1j)~=0))./kount;
end
if 0
up(poly_ij)=min(u(edge_u_ij),u(edge_u_ijm1));
vp(poly_ij)=min(v(edge_v_ij),v(edge_v_im1j));
end
if 0
up(poly_ij)=0.5*(u(edge_u_ij)+u(edge_u_ijm1));
vp(poly_ij)=0.5*(v(edge_v_ij)+v(edge_v_im1j));
end
if 1
a=u(edge_u_ij);
b=u(edge_u_ijm1);
...
%up(poly_ij(2:end))=0.5*(up(poly_ij(1:end-1))+up(poly_ij(2:end)));
%vp(poly_ij(2:end))=0.5*(vp(poly_ij(1:end-1))+vp(poly_ij(2:end)));
