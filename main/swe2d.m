...
for n=1:nsteps
  ...
  if(((exist('BEDLOAD','var') & BEDLOAD) | ...
      (exist('SSC','var') & SSC)))
      rhs_C=zeros(Ni,Nj);
      
      if(exist('EROSION','var') & EROSION)
          rhs_C=rhs_C+ssc_rhs(n,dt,C,u_old,v_old,xc,yc,h_old+d,cellmark);
      end
  ...
  if(exist('BEDLOAD','var') & BEDLOAD)
    ...
    [qs_x,qs_y,qn_x,qn_y,zb] = bedload(u_old,v_old,zb,dx,dy,dt,H,tan_deltas,C,-rhs_C);
  end
